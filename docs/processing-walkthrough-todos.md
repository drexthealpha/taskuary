...
- [ ] Remove the `reply_only` early filing path when `can_reply()` is false.
...
- [ ] Always request draft generation for `reply_only`; do not gate this path on
...
- [ ] When sending is unavailable, omit the send/approve-and-send button and
...
- [ ] Preserve server-side send checks; hiding a UI button is not authorization.
  Always drafting must not enable outgoing replies or automatically send anything.
- [ ] Make draft-generation failures or missing AI configuration visible with a
  retry action; keep the item reply-needed rather than treating it as FYI or
  falsely claiming a draft exists.
- [ ] Test sending-disabled/read-only/missing-permission cases, enabled sending,
...
- [ ] Capture the exact input message IDs/context revision at task evaluation
...
- [ ] Apply a shared source-freshness check before assistant surfacing, discussion,
  ...
  of the entire chain.
- [ ] Select an item first, then validate its freshness, including automatic
...
- [ ] On newly relevant inbound or owner-sent activity, reevaluate the updated
...
- [ ] When new messages are detected on the item being discussed/surfaced, notify
...
- [ ] Tell the owner when new activity changes the item being discussed. If the
...
- [ ] Use the assembled substantive chain for drafting as well as triage; remove
...
- [ ] Recheck the evaluated version before committing an action. Require renewed
...
- [ ] Add regression tests for email/chat refresh, Next without a key, FYI batches,
...
- [ ] Test that the new-message/retriage notice arrives before the triage result,
...
- [ ] Remove general `LEARNED.md` prompt injection from reply drafting and
...
- [ ] Build replies using STYLE.md for voice, phrasing, greetings, and signatures;
...
- [ ] Restrict separately retrieved standing memory notes to explicit,
...
- [ ] Route explicit writing-style feedback to STYLE.md rather than general
...
- [ ] Add model-payload tests for initial drafts, redrafts, and message-only
...
- [ ] Default email drafts to Reply all; offer Reply to (sender/reply address)
...
- [ ] Persist and display the selected recipient envelope with the draft so
...
- [ ] Apply the appropriate owner's email signature once in the draft, visible
...
- [ ] Test default Reply all, Reply to selection, Reply-To headers, editable To/CC,
...
- [ ] Owner-approved on 2026-09-05: a task with missing/uncertain agent kind
...
- [ ] Test missing/invalid kind and uncertain classification across the supported
...
- [ ] Extend automatic dispatch to general agents, using a per-task worker
...
- [ ] Expose clear auto-start controls for coding and general agents in Settings,
...
- [ ] Apply appropriate shared safety/permission checks to both worker kinds,
...
- [ ] Dispatch once per eligible work item, not per refresh or historical chain
...
- [ ] Test both default auto-start paths, each settings opt-out, manual fallback,
...
- [ ] Extend the same triage verdict with a meaningful task title/summary and
...
- [ ] Persist the checklist with the task and render interactive, saved checkboxes
...
- [ ] Give checklist items stable identity so edits, progress, and fresh-triage
...
- [ ] Keep checklist progress separate from agent/session state and task completion;
...
- [ ] Add tests for multi-request chains, coding/general/personal tasks, meaningful
...
- [ ] Remove `store.known_sender()` / "has written before" as an authorization
...
- [ ] Retain verified Sent Items evidence that the receiving mailbox previously
...
- [ ] Expose same-domain trust and non-email-channel trust behavior explicitly in
...
- [ ] Apply the configured sender-trust contract consistently to coding and
...
- [ ] Add tests for prior incoming-only history remaining untrusted, verified sent
...
- [ ] Owner-approved: count all live coding/general agent sessions toward the
...
- [ ] Owner-approved: allow at most two automatic retries after the initial
...
- [ ] Configuration, missing repository/worker, and permission failures should
...
- [ ] After retry exhaustion, retain the task and dispatch error visibly but stop
...
- [ ] Schedule due retries without depending solely on an unrelated session ending;
...
- [ ] Test transient recovery, exactly three failed attempts, persisted backoff
...
- [x] Move `clear_dispatch()` inside the startup `try`, after `start_on_task()`
...
- [x] Add a regression test proving a failed start remains queued and a later
...
- [ ] Supply relevant known projects, repositories, and people associations with
...
- [ ] An explicit owner-selected task repository wins. Treat sender/project
...
- [ ] Validate returned project/repository IDs against the supplied known
...
- [ ] Before coding startup, validate/resolve the selected local checkout. Missing
...
- [ ] Test explicit override precedence, mapped person with a matching request,
...
- [ ] Use a consistent pre-launch repository picker on the task page, timeline
...
- [ ] Cover personal-task/general-to-coding corrections and manual coding dispatch;
...
- [ ] Preserve the selected item, agent/model, and instructions while choosing;
...
- [ ] Add tests for all three UI entry points, conversational coding requests,
...
- [ ] Use one canonical item/identity/context representation for All, Unread,
...
- [ ] All sorts by time (newest first). Unread selects the unread subset and
...
- [ ] Remove Unread-only age windows, category exclusions, and silent caps that
...
- [ ] Do not treat an FYI classification or merely surfacing an item as permission
...
- [ ] Show newly persisted arrivals in both views immediately when eligible,
...
- [ ] Replace the incomplete pipeline revision/update comparison: changes to
...
- [ ] Superseding the earlier separate-filter proposal: remove the Needs me tab.
...
- [ ] Apply all common source/category/mute exclusions identically to All and
...
- [ ] Test All/Unread item parity (after read filtering), time versus status order,
...
- [ ] Put genuinely immediate/time-critical items first (a current calendar
...
- [ ] Put tasks actively worked by either coding or general agents in band 5,
...
- [ ] Test coding/general working placement, transitions to owner-waiting, and
...
- [ ] Preserve the existing funnel UI during this selection refactor: keep its
...
- [ ] Remove assistant-local filtering, ranking, and eligibility recalculation:
...
- [ ] Clicking "Walk me through my tasks" resumes a valid Current item; only
...
- [ ] Replace next_item()'s surfaced-based exclusion and preference for unshown
...
- [ ] Skip working agents, deferred items, and pending-triage items for automatic
...
- [ ] Track Current explicitly: once selected, keep that item as the conversation
...
- [ ] Preserve the approved four-at-a-time FYI presentation for automatic walks;
  batching must follow shared ordering rather than introducing another ranking.
- [ ] Test displayed Unread versus assistant selection, already-shown but unread
...
- [ ] Remove decide_words() and other phrase/regex-based intent dispatch from
...
- [ ] Give every message the same conversational action interface. Replace the
...
- [ ] Before executing a requested action (except draft preparation and Next navigation below), show a confirmation box describing
...
- [ ] Except for the explicitly approved draft-preparation and Next-navigation exceptions below, no
...
- [ ] Use one validated action execution path shared with other app entry points.
  ...
  or advance the walk.
- [ ] Preserve draft review/editing and explicit send approval within this model.
...
- [ ] Test typed/suggested-prompt parity, no phrase-dispatch bypass, clarification,
...
- [ ] Owner-approved navigation exception: an unambiguous request to move Next
...
- [ ] After a confirmed owner action succeeds, compare its outcome with the
...
- [ ] Record source/message or grouped-item identity, triage verdict/revision,
...
- [ ] Preserve the distinction between correction evidence, explicit learned
...
- [ ] Record evidence against source items even when an FYI has no task. Persist
...
- [ ] When an FYI later becomes a task, link its earlier relevant discussion and
...
- [ ] Test successful corrections from every entry point, unchanged verdicts,
...
- [ ] Keep the delegated task visible in Unread as Working; do not settle it as
...
- [ ] Test successful handoff advances once, retains the working task in Unread,
...
- [ ] Receive structured input/approval-needed events from worker integrations,
...
- [ ] Let the owner answer conversationally in the assistant. Clarify ambiguous
...
- [ ] Bind delivery to the specific outstanding request and run, never merely the
...
- [ ] Use a supported reply/control path per worker integration; lifecycle hooks
...
- [ ] Distinguish queued, delivered, and resumed/working states. Do not report
...
- [ ] Test general/coding question relay, multiple waiting agents, request/run
...
- [ ] Check connector sending capability/permissions before offering send. Keep
...
- [ ] Confirmed send closes the task; definite failure preserves the draft and
...
- [ ] When sending is unavailable, offer a separate confirmed Close without
...
- [ ] Test read-only/missing permissions, explicit close-without-send and cancel,
...
- [ ] Apply successful reply -> task done consistently across assistant approval,
...
- [ ] Drafting, editing, approving without confirmed send success, failed/blocked
...
- [ ] Refresh task, review, and canonical Unread state after confirmed send and
...
- [ ] Test successful send closes even with unchecked task TODOs, failed sends
...
- [ ] Present up to four FYIs together with a summary for each. Each entry is
...
- [ ] For a single task item, show the full message/context and the task summary
...
- [ ] Generate the assistant's explanation according to COUNSEL, removing the
...
- [ ] Presenting either kind of card does not mark it read or handled and does
  not automatically execute actions or advance the conversation.
- [ ] Test four-item FYI presentation, per-item action targeting, untouched sibling
...
- [ ] New chat archives the current conversation, resets chat/Current, and opens
...
- [ ] Listing/opening earlier chats is read-only and paginated. Remove the
...
- [ ] Implement automatic retention cleanup as a separate scheduled lifecycle
  operation with a chat retention setting defaulting to 15 days, not the existing
  hardcoded 20-day cutoff.
- [ ] Preserve task-linked discussion, confirmed actions, agent results, and send
...
- [ ] Keep correction memory and saved rules on independent lifecycles. Chat
  deletion must not erase learning evidence, learned preferences, or exclusions.
- [ ] Test New chat archives without deletion or auto-walk, history reads do not
...
- [ ] Persist Current explicitly and validate it against current canonical item
...
- [ ] Keep history/pipeline loading separate from initiating an assistant turn.
...
- [ ] Test first visit, returning from another tab, page reload/remount, stale
...
- [ ] Owner-approved notification placement: unsolicited updates use a single
...
- [ ] Keep the strip until Open, Later, or resolution; do not rely on a timed
...
- [ ] Test no automatic chat insertion for Current and non-Current updates,
...
- [ ] Remove event-driven clearing/replacement of Current and scheduled surface()
...
- [ ] Only explicit owner navigation/actions may advance under their agreed
...
- [ ] Test updates on Current, unrelated urgent arrivals, delayed event delivery,
...
- [ ] Remove automatic deferral behind another task solely because of likely file
...
- [ ] Before startup, give the agent current same-checkout peer task IDs, agent
...
- [ ] Refresh coordination context as peers start, change work, or stop. Closed
...
- [ ] If AI overlap assessment is absent, uncertain, or fails, still provide the
...
- [ ] Test similar tasks launching when capacity permits, both dispatch modes,
...
- [ ] Explicitly remove the deliberate closed-session wall-note injection from
...
- [ ] Apply the same removal explicitly to general-assistant context:
...
- [ ] Associate coordination notes with a specific agent session/run, not merely
...
- [ ] Use the same live-note selection for the board UI, agent startup context,
...
- [ ] Retain ended-run notes in task/run history, explicitly historical and not
...
- [ ] Test active and approval-waiting runs, completed/stopped runs, same-task
...
- [ ] Define shared AGENT.md operating rules for both worker kinds: task scope,
...
- [ ] Build one authoritative task brief containing task ID, objective, triage
...
- [ ] Stop injecting the full SOUL.md into coding and general-worker prompts.
...
- [ ] Consolidate relevant source rules, playbook guidance, and saved preferences
...
- [ ] Add live coordination only when relevant active peers/notes exist, using
...
- [ ] Test both worker kinds for shared rules, coding-only additions, absence of
...
- [ ] Simplify initial onboarding to name, then AI coding CLI selection/setup
...
- [ ] Add a discoverable system-setup walkthrough entry point at the top Assistant
...
- [ ] Ship a Taskuary setup skill containing the product-specific setup procedure,
...
- [ ] Reuse direct report/connection setup and shared confirmed configuration
...
- [ ] Support resuming incomplete setup and revisiting individual setup areas
...
- [ ] Test first-run name/CLI handoff, missing CLI readiness, top-level setup entry,
...
- [ ] Interpret setup requests through AI, gather missing configuration, and show
...
- [ ] For reports, gather source/connection, query or inputs, summary instructions,
...
- [ ] For connections, gather provider and non-secret configuration, show requested
...
- [ ] If setup requires substantial investigation, propose a general-agent setup
...
- [ ] Test conversational clarification/correction/cancel, exact configuration
...
- [ ] Replace assistant idea direct feed-route classification with shared triage
...
- [ ] Use the resulting classification and importance for canonical Unread order,
...
- [ ] Preserve the existing opt-in report triage capability: configured reports
...
- [ ] Test informational/actionable/urgent ideas, linked active work, duplicate
...
- [ ] Separate scheduled/configured workflow definitions from reusable procedures
...
- [ ] Dispatch a triggered workflow directly to a general agent with the workflow
...
- [ ] Keep incoming-request procedure matching in unified triage (for example,
...
- [ ] Deliver the selected procedure to either worker kind through the shared
...
- [ ] Preserve existing definitions when separating these concepts; explicitly
...
- [ ] Test scheduled and manual workflow runs bypassing message triage and
...
- [ ] Apply that startup contract to coding and general workers: distinguish
...
- [ ] Verify equivalent Make task and Send to agent behavior across All timeline
...
- [ ] Audit finding: All detail's SendToAgent is hidden for unconverted
...
- [ ] Audit finding: ui.jsx SendToAgent treats a successful HTTP response with
...
- [ ] Audit finding: TasksView.startGeneralAgent dispatches existing general
...
- [ ] Test the actual All-detail buttons and matching chat/Tasks actions: owner
...
- [ ] Route task-view controls and assistant confirmation boxes through the same
...
- [ ] Unify coding startup (currently Kind PATCH plus openTerm) and general
...
- [ ] Make task completion versus agent completion explicit in labels/help and
...
- [ ] Reconcile Finish agent run and Save stopped run result with the approved
...
- [ ] Clarify Pause & save means end the session with saved continuation context,
...
- [ ] Verify Write reply, Generate reply, and Ask sender create/open the intended
...
- [ ] Add handler/backend tests plus actual UI interaction coverage for these
...
- [ ] Model Working, Input needed (with the unanswered question), Approval needed
...
- [ ] Integrate Claude Code lifecycle/tool hooks for prompt submission, permission
...
- [ ] Integrate Codex App Server structured turn lifecycle, approval, and user-input
...
- [ ] Emit regular API-agent lifecycle events directly from the execution loop.
...
- [ ] Separate response/turn completion from work completion. Require an explicit
...
- [ ] Persist and reconcile events by task/run/turn and request IDs. Deduplicate
...
- [ ] Keep Working visible in Unread without injecting it into chat. Promote
...
- [ ] Test all providers: long silent work, terminal repaint noise, questions,
...
- [ ] Capture the agent's actual final answer directly, rather than requiring a
...
- [ ] Persist the final Markdown result, evidence/artifact references, and reported
...
- [ ] Replace the manual-start stay-open veto for explicit successful completion.
...
- [ ] If result persistence fails, do not discard the session or claim successful
...
- [ ] Test automatic and manually started workers, matching final-answer capture,
...
- [ ] Apply the approved freshness and unified reevaluation rules after work
...
- [ ] Reuse the applicable held review rather than duplicating it. Generate the
...
- [ ] Bring coder.finish() into line with the approved always-draft rule: when
...
- [ ] Test changed requests, already-sent external replies, unchanged chains,
...
- [ ] Owner clarification: when new context materially changes the task/draft and
...
- [ ] Base approval invalidation on the draft's context/triage revision and
...
- [ ] Test material inbound changes, externally sent replies, relevant triage
...
- [ ] Remove shared COUNSEL injection from assistant.think() report-generation
...
- [ ] Audit other COUNSEL consumers, including suggestion discussions and general
...
- [ ] Test that changing chat COUNSEL does not alter the scheduled report prompt,
...
- [x] Implemented the owner-approved COUNSEL loader change in concierge._counsel:
...
- [x] Added loader regressions for complete long instructions reaching _system,
...
- [x] Applied approved consistency pass to shipped/live COUNSEL: shared Unread
...
- [ ] Owner confirmed the boundary: retain the minimal machine action contract
...
- [x] Applied approved Voice section to shipped and live saved COUNSEL: concise
...
- [x] Replaced "The post" in shipped and live saved COUNSEL with the approved
...
- [x] Applied approved urgent-update wording to shipped and live saved COUNSEL:
...
- [x] Applied the approved named-item rule to the shipped and live saved COUNSEL:
...
- [x] Owner requested immediate implementation of the first responsibility in
...
- [x] Owner approved the walkthrough/FYI/overview rule: discuss Current and wait
...
- [x] At the owner's explicit request, applied both approved COUNSEL edits to the
...
- [ ] Remove concierge.SYSTEM's competing behavioral/routing instructions and
...
- [ ] Keep authorization, valid action/argument checks, target validation,
...
- [ ] Remove silent 3,200-character truncation of COUNSEL; apply explicit size
  validation/budget handling so approved instructions are not silently dropped.
- [ ] Audit remaining inline prompts/action heuristics and canned receipts for
...
- [ ] Test that editing the assistant document affects the actual loaded prompt,
...
- [ ] Fix desktop shutdown's daemon-thread race: signal backend shutdown and wait
...
- [ ] Leave unfinished work open and visibly interrupted, not Working or Done.
...
- [ ] Handle stuck cleanup with visible progress/error and a bounded escalation
...
- [ ] Test native desktop quit waits for server cleanup, transcript/history
...
- [ ] Trace browser ownership/control and actual UI with the owner: which agent
  owns it, when it opens, visibility of actions, manual takeover, and its relation
  to the routing chat and task workspace.
- [ ] Review navigation versus consequential website actions, confirmation,
...
- [ ] Agree the UI/interaction contract before redesigning browser control; then
